    ...
    >
      <div className="grid gap-3">
        {frameworks.map(({ id, title, body, tag, icon: Icon }) => {
          ...
          return <button key={id} type="button" onClick={() => select(id)} aria-pressed={selected} className={`option-card text-left ${selected ? 'option-card-selected' : ''}`}><div className="flex items-start gap-3"><span className="icon-tile shrink-0"><Icon className="h-4 w-4" /></span><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><h3 className="font-medium text-ink">{title}</h3>{selected && <span className="badge-success"><Check className="h-3 w-3" />{t.selected}</span>}</div><p className="mt-2 text-xs leading-relaxed text-ink-muted">{body}</p><span className="badge-neutral mt-3 inline-flex">{tag}</span></div></div></button>;
        })}
      </div>
    </Modal>
  );
